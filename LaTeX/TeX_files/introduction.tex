...
a two dimensional $M$ orbifold as a list: 
\begin{equation}
\times \underbrace{\circ\cdots\circ}_{c\ \mathrm{times}}r_1\cdots r_n 
*d_1^1\cdots d_{m_1}^1 \cdots *d_1^b\cdots d_{m_b}^b,
\end{equation}
where $\times$ is present only if $M$ is non-orientable, each $\circ$ represent one handle, 
...
We will also view any two dimentional orbifold as a $S^2$ with added features listed above:
\begin{itemize}
\item cross-caps
\item handles
