...
What is interesting, these are the same results 
...
This presentation is not necessarily unique, as $\circ^a\times^b = \times^{2a+b}$. For a more 
...
:
\begin{itemize}
\item if $\cho{O} > 0$ -- an elliptic structure,
\item if $\cho{O} = 0$ -- a parabolic structure,
\item if $\cho{O} < 0$ -- a hyperbolic structure.
\end{itemize}
