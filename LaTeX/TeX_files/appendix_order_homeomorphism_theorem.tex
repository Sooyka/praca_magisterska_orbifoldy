...
\begin{lemma}
For two well ordered sets $A, B \subseteq \mathbb{R}$, with order types, respectively 
$\omega^m + 1$ 
and $\omega^n + 1$, 
...
there is some proper prefix of $\gamma$ with order type $\omega^n$. Let us name this 
prefix $C$.  
...
Let $b_0 \in \mathbb{B}$ and $\epsilon > 0$ be such that $\forall_{c \in C}\ c<b_0-\epsilon$. 
From \ref{accumulation_points_and_order} we know, that 
$C$ has an accumulation point of order $n$. This gives us a contradiction with \ref{} though, as 
the only accumulation point of order $n$ in $B$ is greater than $a$
