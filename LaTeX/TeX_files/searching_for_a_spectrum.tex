...
for a given state $D$, that at most 22222, 
all the states $D'\preceq D$ have some state $D''$, such 
that $d'\preceq D''$, and that $D''$ was the state of the counters at some point. 

$\bullet$ For the fist state we have it is true since it is the $D''$ for all the previous ones.   

$\bullet$ Let us suppouse that for a state $D = d_1d_2d_3\cdots$ we have aour induction thesis. 
We will go through several cases. 
\begin{enumerate}
\item $D$ is a successor of some $D'$. There are two options -- either $D'$ was a state of counters 
at some point or it wasn't. If it wasn't from the inductive assumption we know 
that some state equal at least $D$ was the state of counters.

In the case it was a state of counters --  three cases
here there will be 222222 as an exeption


\item $D$ was not a cuccessor of any $D' \preceq D$. From this, we have that $D$ has at least one 
$\infty$ on its couters. \ref all are on the left. 
Let $c_l$ be the counter that has rightmost infty in $D$. 

We know that for any smaller there is one . Let take the smallest ones for each. if any of them 
...
Now we will show that at some point, lgorithm will have as a state of counters exactly 222222. 
