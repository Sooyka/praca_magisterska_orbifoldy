...
\end{theorem}
\subsubsection{Proof.}
From \ref{sufficiency of D2 and S2} we know, that to check if $-\frac{1}{84}$ is maximal 
...
Let us observe, that for any $D^2$ orbifold that have $5$ orbipoints or more, have an \Eoc 
...
negative \Eoc among orbifolds, respectively, with four orbipoints, without any 
...
Let us observe, that 
the orbifold fith the greatest negative \Eoc, that 
...
the form $*2\ 3\ n$ and it has to be unique. 
The one with the biggest \Eoc\ among them is $*2\ 3\ 7$. 

Let us observe that no $D^2$ orbifold with rotational orbipoints can have such \Eoc. 
For the sake of contradiction, let us assume that there is some orbifold 
$r_1 \cdots r_n * d_1 \cdots d_m$, with \Eoc\ equal to $-\frac{1}{84}$. 
...
and it have no repeted degree, so it can't bo of the form $*r_1r_1 \cdots r_nr_nd_1\cdots d_m$ 
with $n\neq 0$. 
...
for any other two dimensional manifold $M$. 
We can rule out all manifolds with Euler characteristic $\leq 0$ as orbifolds 
corresponding to them have \Eoc at most $-\frac{1}{4}$. 
...
rotational orbipoints. $_\square$ 
