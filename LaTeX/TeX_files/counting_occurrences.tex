...
into two parts. The answears to these partial questions will be given in \ref{algorithm} and 
\ref{}.   
...
\begin{enumerate}
\item For now we only considered sums coresponding to 
...
arithmetical question concerning the sums coresponding to both types 
of orbipoints. It is possible to reduce (in the sense discussed in \ref{})
...
are not neccesery the same (see \ref{sameness}). 
\end{enumerate}
This are the two , 
that each orbifold correspond to only one sum from the point 2., but in the case 
where $M$ has a boundary, multiple orbifolds correspond to the same sum. 
This gives us that the total number 
\end{enumerate}
