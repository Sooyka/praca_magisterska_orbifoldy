...
Using \ref{spe_M} and \ref{}
we conclude that the problem of deciding whether $\frac{p}{q}$ is in $\spe(M)$
is equivalent to deciding whether $some expresion$ is in $\sdD$. 
...
We want to determine whether there exists $d_1,d_2,\cdots,d_k$, such that $\chi^{orb}(*d_1\dots d_k) 
= 
\frac{p}{q}$. 
