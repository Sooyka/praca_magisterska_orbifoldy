...
for any $x \in \sigma$, fr any two dimensional manifold $M$, there are only 
finitely many $M$-orbifolds with only dihedral orbipoints, 
with the \Eoc\ equal to $x$.
\end{observation}
\subsubsection{Proof.}
...
Let $\{a_n\}$ be the sequence such that $a_n = \Delta(d^0_{i_n})$.
